		...
		switch e.expr {
			...
			case EWhile(econd, e1, normalWhile):
				...
				var continueBody = process(econd, ctx, function(transformed)
					return EIf(transformed, macro __do(), breakName.resolve().call()).at(econd.pos)
				);
				var breakBody = next(emptyExpr());
				if (normalWhile)
					return macro {
						var __doCount = 0;
						function $breakName() $breakBody;
						function $continueName() {
							function __do() {
								function __break() ${breakName.resolve()}();
								function __continue() ${continueName.resolve()}();
								if (__doCount++ == 0)
									do $doBody
									while (--__doCount != 0);
							}
					...
					return macro {
						...
						function $breakName() $breakBody;
						function __do() {
							function $continueName() $continueBody;
							function __break() ${breakName.resolve()}();
							function __continue() ${continueName.resolve()}();
							if (__doCount++ == 0)
								do $doBody
